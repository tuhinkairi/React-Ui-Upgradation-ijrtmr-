...
const IndexingPartners: React.FC = () => {
  ...
  return (
    <section className="py-12 pt-8 px-6 lg:px-16 bg-white text-center">
      <h2 className="text-2xl md:text-3xl font-semibold  mb-12">Indexing And Abstracting</h2>
      <div className="grid grid-cols-5 items-center justify-center gap-x-10 gap-5 max-w-6xl mx-auto">
        {indexData && indexData.map((partner) => (
          <a href={partner.indexing_url} className='inliine-block flex items-center justify-center'>
            <img
              key={partner.indexing_id}
              src={partner.indexing_image_url}
              alt={partner.indexing_name}
              className="w-40 object-contain hover:scale-110 transition ease-in-out"
            />
          </a>
        ))}
      </div>
