...
export default function LatestPublication() {
  return (
    <div className="max-w-md mx-auto bg-white rounded-2xl shadow-[0px_0px_40px_5px_rgba(0,0,0,0.08)] p-6 space-y-6">
      <h3 className="text-center text-lg xl:text-xl capitalize font-semibold text-dark">Latest Publication</h3>

      {publications.map((pub, index) => (
        <div key={index} className="space-y-2">
          <p className="text-sm 2xl:text-base text-gray-800">{pub.title}</p>
          <p className="text-sm 2xl:text-base font-medium text-orange-500">Paper ID : {pub.id}</p>
